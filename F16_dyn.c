...
/*                                                                       */
/* File "F16_dynam.c"                                                    */
/* Version 1.1a by E.R. van Oort & L. Sonneveldt                         */
...
/*																		 */
/*  State variables:													 */
/*   1   Vt          total airspeed                              [m/s]	 */
/*   2  beta        angle of sideslip                           [rad]	 */
/*   3  alpha       angle of attack                             [rad]	 */
/*   4  phi         roll angle                                  [rad]    */
/*   5  theta       pitch angle                                 [rad]    */
/*   6  psi         yaw angle                                   [rad]    */
/*   7  p_body      roll angular rate                           [rad/s]	 */
/*   8  q_body      pitch angular rate                          [rad/s]	 */
/*   9  r_body      yaw angular rate                            [rad/s]	 */
/*   10 x_earth     x position                                  [m]		 */
/*   11 y_earth     y position                                  [m]		 */
/*   12 z_earth     z position                                  [m]		 */
/*   13 pow         power level (0-100%)              [-]       [-]      */
/*																		 */
/*  State derivatives:													 */
...
/* Work Variables */
#define C1              ssGetRWork(S)[0] // (S)->work.rWork 
...
#define rtd           57.29577951 //弧度转角度系数，180/π
...
{
    /*S-function模块的数据属性配置 Simulink 调用mdlInitializeSizes 来配置S-function模块的数据属性：
    S-function模块对话框的参数个数及数据类型*/
    ssSetNumSFcnParams(S, 1);  /* Number of expected parameters */

    ssSetNumContStates(S, 13);//连续状态变量13个
    ssSetNumDiscStates(S, 0);//离散状态变量0个

    ssSetNumInputPorts(S, 3);//输入端口数量3个
    ssSetInputPortWidth(S, 0, 4);//0号输入端口宽度为4维
    ssSetInputPortWidth(S, 1, 1);//1号输入端口为1维
    ssSetInputPortWidth(S, 2, 1);//2号输入端口为1维

    /* ssSetInputPortDirectFeedThrough(S, 1, 1); */

    ssSetNumOutputPorts(S, 1);//输出端口数量1个
    ssSetOutputPortWidth(S, 0, 13);//0号输出端口，宽度13

    ssSetNumSampleTimes(S, 1);//采样时间个数，1个
    //ssSetNumRWork(S, 23);//实数型工作变量，23个，(S)->sizes.numRWork = 23
     ssSetNumRWork(S, 24);//实数型工作变量，24个，(S)->sizes.numRWork = 24,前面工作区变量定义了24个，此处由lxs修改为24个。
    ssSetNumIWork(S, 0);//不用整型工作变量
    ssSetNumPWork(S, 0);//不用指针型工作变量
    ssSetNumModes(S, 0);//不用模式工作变量
    ssSetNumNonsampledZCs(S, 0);//不使用非采样过零检测
...
#define MDL_START 
  ...
  {
        real_T *x = ssGetContStates(S); //获取指向连续状态向量的指针，(S)->states.contStates
        int i;
        
        /* Initialize the state vector */
        for (i = 0; i < ssGetNumContStates(S); i++)//ssGetNumContStates(S),获取状态向量个数，(S)->sizes.numContStates，该值在初始化阶段void mdlInitializeSizes(SimStruct *S)被设置为13个 
        {
            x[i] = mxGetPr(ssGetSFcnParam(S, 0))[i];    
             //ssGetSFcnParam(S, 0)等价于((const mxArray *) _ssGetSFcnParam(S,index)),
            //simstruc_implement.h, _ssGetSFcnParam(S,index)=>(S)->sfcnParams.dlgParams[index]
            //
        }
  }
#endif 
...
#define MDL_DERIVATIVES  
  ...
  {
    real_T *x  = ssGetContStates(S);//获取指向连续状态向量的指针，(S)->states.contStates
    real_T *dx = ssGetdX(S);//获取S结构体内的状态导数指针，(S)->states.dX
    
    InputRealPtrsType u = ssGetInputPortRealSignalPtrs(S, 0);
// ( (InputRealPtrsType) ssGetInputPortSignalPtrs(S,ip) )=>  ( (S)->portInfo.inputs[(ip)].signal.ptrs ) 
